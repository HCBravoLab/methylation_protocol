...
```{r,eval=F}
require(epivizr)
mgr <- startEpiviz(workspace="U2YaLjNoR2",tryPorts=TRUE)
```
...
pvals <- mgr$addMeasurements(dmp.gr, "P-Values (-log10) of
...
beta.tissue <- mgr$addDevice(cpg.gr,"Percent Methylation",type="bp",columns=
...
hyper.dmr <- mgr$addDevice(dmr.gr[which(dmr.gr$type=="hyper")], "Hyper-methylated
  Regions")
```
...
diff.meth.promoters <- mgr$addDevice(dmr.promoters, "Differentially Methylated
  Promoters")
```
...
meth.diff <- mgr$addDevice(cpg.gr,"Methylation Beta Difference",type="bp",
...
hypo.blocks <- mgr$addDevice(blocks.gr[which(blocks.gr$type=="hypo")],
  "Hypo-methylated Blocks")   
hyper.blocks <- mgr$addDevice(blocks.gr[which(blocks.gr$type=="hyper")],
  "Hyper-methylated Blocks")
```
